...
class InputFeatures(object):
	...
	def __init__(self,
		...
		self.input_chars_ids_a = input_chars_ids_a
		self.input_mask_a = input_mask_a
		self.input_ids_b = input_ids_b
		self.input_chars_ids_b = input_chars_ids_b 
...
class BiNliProcessor(DataProcessor):
	"""Processor for the MRPC data set (GLUE version)."""
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
	...
	if isinstance(example, PaddingInputExample):
		return InputFeatures(
				input_ids_a=[0] * max_seq_length,
				input_mask_a=[0] * max_seq_length,
				input_ids_b=[0] * max_seq_length,
				input_mask_b=[0] * max_seq_length,
				input_chars_ids_a = [0] * max_seq_length,
	...
	if tokenizer.use_pretraining:
		assert len(input_chars_ids_a) == max_seq_length
		assert len(input_chars_ids_b) == max_seq_length
	...
	feature = InputFeatures(
			...
			input_chars_ids_a=input_chars_ids_a,
			input_mask_a=input_mask_a,
			input_ids_b=input_ids_b,
			input_chars_ids_b=input_chars_ids_b,
...
def file_based_convert_examples_to_features(
	...
	for (ex_index, example) in enumerate(examples):
		...
		features["input_chars_ids_a"] = create_int_feature(feature.input_chars_ids_a)
		features["input_ids_b"] = create_int_feature(feature.input_ids_b)
		features["input_mask_b"] = create_int_feature(feature.input_mask_b)
		features["input_chars_ids_b"] = create_int_feature(feature.input_chars_ids_b)
...
def create_model(bert_config, is_training, premise_input_ids, premise_input_chars_ids, premise_input_mask,
	...
	probabilities = tf.nn.softmax(output_layer, axis=-1)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
					 ...
					 use_one_hot_embeddings, use_pretraining):
	...
	def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
		...
		input_ids_premise = features["input_ids_a"]
		input_mask_premise = features["input_mask_a"]
		input_ids_hypothesis = features["input_ids_b"]
		input_mask_hypothesis = features["input_mask_b"]
		input_chars_ids_premise = features["input_chars_ids_a"]
		input_chars_ids_hypothesis = features["input_chars_ids_b"]
...
def main(_):
	...
	model_fn = model_fn_builder(
			...
			use_pretraining=FLAGS.use_pretraining)
